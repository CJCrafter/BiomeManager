...
public class v1_19_R3 implements BiomeCompatibility {
    ...
    public v1_19_R3() {
        ...
        for (Biome biome : biomes) {
            ...
            try {
                ...
                BiomeRegistry.getInstance().add(key, new BiomeWrapper_1_19_R3(biome));
            } catch (Throwable ex) {
    ...
    public BiomeWrapper createBiome(NamespacedKey key, BiomeWrapper base) {
        return new BiomeWrapper_1_19_R3(key, (BiomeWrapper_1_19_R3) base);
    }
    ...
    public BiomeWrapper getBiomeAt(Block block) {
        ...
        if (wrapper == null)
            wrapper = new BiomeWrapper_1_19_R3(getBiome(key));
    ...
    public void handleChunkBiomesPacket(PacketEvent event) {
        ...
        LevelHeightAccessor heightAccessor = ((CraftWorld) event.getPlayer().getWorld()).getHandle();

        int counter = 0;
        FriendlyByteBuf sectionBuffer = chunkData.getReadBuffer();
        for (int i = 0; i < ySections; i++) {
            sections[i] = new LevelChunkSection(heightAccessor.getSectionIndex(i), registry);
            ...
            for (int x = 0; x < 4; x++) {
                for (int y = 0; y < 4; y++) {
                    for (int z = 0; z < 4; z++) {
                        ...
                        BiomeWrapper biome = BiomeRegistry.getInstance().get(namespace);
                        biomes[counter++] = biome;

                        if (biome == null)
                            throw new IllegalStateException("Biome '" + namespace + "' does not exist in BiomeManager's registry");
                    }
                }
            }
        }
